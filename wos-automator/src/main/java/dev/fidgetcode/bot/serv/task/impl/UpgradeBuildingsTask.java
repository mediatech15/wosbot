...
import static dev.fidgetcode.bot.console.enumerable.EnumTemplates.BUILDING_BUTTON_INFO;
import static dev.fidgetcode.bot.console.enumerable.EnumTemplates.BUILDING_BUTTON_SPEED;
import static dev.fidgetcode.bot.console.enumerable.EnumTemplates.BUILDING_BUTTON_TRAIN;
import static dev.fidgetcode.bot.console.enumerable.EnumTemplates.BUILDING_BUTTON_UPGRADE;
import static dev.fidgetcode.bot.console.enumerable.EnumTemplates.BUILDING_SURVIVOR_BUTTON_UPGRADE;
import static dev.fidgetcode.bot.console.enumerable.EnumTemplates.GAME_HOME_SHORTCUTS_HELP_REQUEST;
import static dev.fidgetcode.bot.console.enumerable.EnumTemplates.GAME_HOME_SHORTCUTS_HELP_REQUEST1;
import static dev.fidgetcode.bot.console.enumerable.EnumTemplates.GAME_HOME_SHORTCUTS_OBTAIN;
import static dev.fidgetcode.bot.serv.task.constants.ButtonConstants.*;
import static dev.fidgetcode.bot.serv.task.constants.LeftMenuTextSettings.*;
...
public class UpgradeBuildingsTask extends DelayedTask {
    ...
    protected void execute() {
        ...
        if (hasIdleQueue) {
            ...
            for (QueueAnalysisResult result : queueResults) {
                if (result.state.status == QueueStatus.IDLE ||
                        result.state.status == QueueStatus.IDLE_TEMP) {
            ...
            List<QueueAnalysisResult> updatedResults = analyzeAllQueues();
            ...
            if (!troopTrainingTimes.isEmpty()) {
                ...
                for (LocalDateTime trainingTime : troopTrainingTimes) {
                    ...
                    QueueState trainingState = new QueueState(
                            QueueStatus.BUSY, timeString);
                    QueueAnalysisResult trainingResult = new QueueAnalysisResult(
     ...
     *         null otherwise
     */
    private LocalDateTime processQueue(QueueAnalysisResult queueResult) {
