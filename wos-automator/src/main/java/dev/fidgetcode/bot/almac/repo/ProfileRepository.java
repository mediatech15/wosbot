...
import dev.fidgetcode.bot.ot.DTOConfig;
...
public class ProfileRepository implements IProfileRepository {
	...
	public List<DTOProfiles> getProfiles() {
		String queryProfiles = "SELECT new dev.fidgetcode.bot.ot.DTOProfiles(p.id, p.name, p.emulatorNumber, p.enabled, p.priority, p.reconnectionTime, p.characterId, p.characterName, p.characterAllianceCode, p.characterServer) FROM Profile p";
		...
		if (!profileIds.isEmpty()) {
			...
			String queryConfigs = "SELECT new dev.fidgetcode.bot.ot.DTOConfig(c.profile.id, c.key, c.value) " + "FROM Config c WHERE c.profile.id IN :profileIds";
	...
	public DTOProfiles getProfileWithConfigsById(Long id) {
		...
		String queryProfile = "SELECT new dev.fidgetcode.bot.ot.DTOProfiles(p.id, p.name, p.emulatorNumber, p.enabled, p.priority, p.reconnectionTime, p.characterId, p.characterName, p.characterAllianceCode, p.characterServer) FROM Profile p WHERE p.id = :id";
		...
		String queryConfigs = "SELECT new dev.fidgetcode.bot.ot.DTOConfig(c.profile.id, c.key, c.value) FROM Config c WHERE c.profile.id = :profileId";
